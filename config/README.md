...
### COMMONURL
`String` - `auth-proxy` использует это значение для запросов к сервису общих данных
...
### DISABLEDMDM_TYPES
`String` as `JSON Array` - указывает потоку `common`, не формировать файлы образов для перечисленных типов
