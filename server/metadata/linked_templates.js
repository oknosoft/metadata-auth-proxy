    ...
    if(job_prm.server.abonents.includes(abonent.id)) {
      res = res.then(() => {
        ...
        .then(() => {
          ...
          const cnames = [doc.calc_order.metadata().synonym, cat.templates.metadata().synonym];
          abonent.acl_objs._obj
            .filter(({type}) => cnames.includes(type))
            .forEach(({obj: ref, type}) => {
              if(type === doc.calc_order.metadata().synonym) {
                const obj = doc.calc_order.get(ref);
                all_tmplts.add(obj);
                obj.is_new() && tmplts.add(obj);
              }
              else if(type === cat.templates.metadata().synonym) {
                const obj = cat.templates.get(ref);
                obj.templates.forEach(({template}) => {
