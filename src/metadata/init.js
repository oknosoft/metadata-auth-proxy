...
* Ресурсы предприятия, загрузку которых необходимо планировать
  ...
  async loadRegister(client, {md, wsql: {alasql}, cat: {work_shifts}, enm: {planning_phases}, utils: {moment}}) {
    ...
    class RowsFragment {
      ...
      reminders(phase, date, shift) {
        ...
        const res = alasql('select date, shift, time, sum(power) power from ? group by date, shift, time', [rows]);
        return res;
      }
    }

    for(const work_center of this) {
      work_center.register = new RowsFragment(work_center);
    }
...
/**
* ### Регистр сведений ПредопределенныеЭлементы
...
/**
* ### Регистр сведений Интернационализация
* Интернационализация
* @class IregI18n
* @extends RegisterRow
* @constructor
